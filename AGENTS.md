...
Owner-only /evidence/ adds private immutable snapshots of the academic journal and explicitly attached original files, standard salted EAS PrivateData multiproofs, selective export, and optional wallet-approved mainnet EAS timestamping. User explicitly accepts optional transaction fees; required study remains free. Do not add other learner profiles, deploy new contracts, issue credentials or send paid transactions automatically. Keep private originals, salts and recovery packages out of public Git and blockchain. Current bounds: 3 MB attached files per snapshot, 8 MB complete package; never truncate oversized evidence.
