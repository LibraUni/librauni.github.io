...
## Tutorial intake, formal submission and incremental proofs — 26 September 2026
...
Desired routine evidence behaviour is incremental: existing full snapshot is baseline; later batches contain only new work and appended corrections, a persistent archive identity, sequence and cryptographic predecessor link. Preserve all historical records/proofs. No changes means no new batch or unnecessary fee. Keep a separate complete-archive export, and self-contained selected exports that include required original bytes from any batch. Monthly batches form one growing archive. Current v2 still downloads full snapshots: do not claim incremental operation until implemented and tested. Next implementation contract: docs/incremental-evidence-and-submissions.md in website. This discussion is platform planning, not an academic journal event.
