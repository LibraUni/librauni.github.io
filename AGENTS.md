...
## Personal evidence and optional Ethereum anchoring — 26 September 2026

Owner-only /evidence/ adds private immutable snapshots of the academic journal and explicitly attached original files, standard salted EAS PrivateData multiproofs, selective export, and optional wallet-approved mainnet EAS timestamping. User explicitly accepts optional transaction fees; required study remains free. Do not add other learner profiles, deploy new contracts, issue credentials or send paid transactions automatically. Keep private originals, salts and recovery packages out of public Git and blockchain. The v1 bounds below were superseded by the tutor-managed v2 workflow; never truncate oversized evidence.

Maintain evidence-based tutor logging with actual source/date limitations. Distinguish snapshot preparation, server save, reported learning time and independently verified blockchain time. An anchor proves a matching commitment, not academic truth, independent authorship, real-world identity, accredited status or completeness of undisclosed history. Wallet-linked records are personal attestations, not independently issued tutor credentials. Corrections append; never replace anchored evidence. Preserve old snapshots/files and predecessor links. Blockchain is not backup storage.

The browser verifier is a convenience; exports use the official EAS multiproof format and include a separately runnable verifier. Do not claim Blockcerts compatibility or an external nontechnical verifier exists for this package. Future actual credentials require a separate assessed-evidence/issuer/revocation design. M100 enrolment remains closed; preparing or anchoring evidence never enrols or awards credit. Consult website/docs/learning-evidence.md (docs/ within website) before changing this feature. Test integrity, privacy, restore and chain-finality/error paths. No website-planning conversation belongs in the academic journal.


## Tutor-managed full student records — 26 September 2026

This supersedes the original manual attachment / Prepare / Save workflow. The tutor owns preservation of every supplied academic original, correction, assessment record and relevant journal entry. Use scripts/archive-student-records.mjs with a private manifest outside public Git; originals remain in private study storage and immutable owner-only Firestore academicFiles chunks. Publish catalogue entries only after byte-for-byte restore verification. Store marks, criteria, feedback, assessment version, assistance conditions, source dates, corrections and links in academicRecords as distinct fields when actually evidenced. Add the substantive academic journal entry through the existing session uploader. Never label summaries as full transcripts or infer certificates/grades from archival. Administrative discussions are not learning events.

The student uses one button at /evidence/: “Generate proof of full student records”. It collects every saved academic entry, tutor register record and archived original, detects inventory changes, verifies originals, saves/restores a private immutable snapshot and downloads the organised ZIP. The button performs deterministic collection, not live AI grading; tutor maintenance must happen beforehand. Preserve missing-source limitations honestly. No new learner accounts are introduced.

Use v2 salted EAS proofs for individual record fields and original-file hashes. Full ZIPs contain readable fields and exact original bytes, category/item proof companions, and private recovery data. Selective ZIPs contain only chosen fields/files and their proofs; snapshot context/counts are not forced into individual disclosures. Files must travel with their proof. Verify missing/changed bytes and readable text, private persistence, pagination, legacy compatibility and independent verifier interoperability. Do not claim a root certifies academic truth, an external issuer, identity or accreditation. Optional Ethereum anchoring remains a separate explicit wallet-approved paid action; never pay automatically. Current limits are documented; oversized evidence must fail clearly without silent omission. Consult docs/learning-evidence.md.


## Tutorial intake, formal submission and periodic full proofs — 26 September 2026

Proactively classify and privately archive every academic file supplied during an actual tutorial (activities, code, drafts, sketches, notes, images, PDFs and other academic artefacts), without asking the learner to reattach it or request archival. Preserve original bytes and versions, source and available dates, module/activity context and draft/final status. Use the existing tutor uploader, verify restoration and add the appropriate academic journal entry. If classification is uncertain, archive it provisionally and refine later; do not discard it. Administrative attachments are not academic evidence. A tutorial draft is not a formal assessment submission.

Formal TMA/EMA/exam work requires an explicit assignment-specific Submit action and immutable receipt (assessment/version, attempt, server time, verified original-file inventory). iCMAs use explicit final submission/receipt. The tutor retrieves submissions from that private route for marking and archives linked corrections/results; do not assume the web button automatically wakes an AI tutor. Submission controls remain gated by released assessment/enrolment readiness. These controls still need implementation.

Final user decision, 26 September 2026: retain the current full-snapshot behaviour. Every generation collects and downloads the entire saved academic inventory and archived originals again. The incremental-batch proposal is withdrawn and must not be implemented. Preserve successive immutable snapshots, their predecessor links and selective sharing. The learner may generate snapshots periodically and explicitly choose which to anchor on Ethereum; generation alone is not an on-chain timestamp and no paid anchoring is automatic. Successive anchors establish successive committed record states, not continuous observation or independent certification of learning. Proactive tutorial archival and the pending dedicated formal Submit route remain required. See docs/incremental-evidence-and-submissions.md for the supersession notice and retained submission requirements. This discussion is platform planning, not an academic journal event.
