...
$('prepare-snapshot').onclick=()=>act(async check=>{
 ...
 const next=buildSnapshot(timeline(records),files,rows.at(-1)?.root||null,new Date().toISOString(),release);check();snapshot=next;saved=false;anchor=null;showSnapshot();message('Prepared locally. Review the list, then Save private snapshot. Nothing has been published or sent to a wallet.');
});
