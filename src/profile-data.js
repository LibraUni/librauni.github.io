...
export const emptyProfile=()=>({schemaVersion:2,...Object.fromEntries(Object.keys(fields).map(k=>[k,''])),photo:'',personalisation:true});
export function validateProfile(p){
 if(!p||p.schemaVersion!==2||Object.keys(p).some(k=>!['schemaVersion','photo','personalisation',...Object.keys(fields)].includes(k)))throw Error('Unrecognised profile format.');
...
// Read old profiles and drafts without rewriting their saved revision or history.
export function readProfile(p){
 if(p?.schemaVersion===1){
  if(typeof p.pronouns!=='string'||p.pronouns.length>80||'surname' in p)throw Error('Unrecognised profile format.');
  const {pronouns,...rest}=p;
  return validateProfile({...rest,schemaVersion:2,surname:''});
 }
