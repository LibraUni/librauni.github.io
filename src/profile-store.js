...
export async function loadProfile(uid,connection=db){const d=await getDocFromServer(doc(connection,'users',uid,'profile','main'));return d.exists()?{revision:d.data().revision,data:readProfile(JSON.parse(d.data().payload))}:{revision:0,data:emptyProfile()};}
