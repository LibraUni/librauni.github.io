...
root.innerHTML=`<div class="planner-heading"><div><p class="eyebrow">YOUR STUDY TIMETABLE</p><h2>${module?'Module planner':'Degree calendar'}</h2></div><button id="planner-auth">Sign in with GitHub</button></div><p id="planner-status" role="status">Sign in to save your personal timetable privately.</p><p id="planner-message" role="alert" hidden></p><div class="planner-tools"><button id="planner-migrate" hidden>Accept programme structure update</button><button id="planner-retry" hidden>Retry saving</button><button id="planner-load" hidden>Load saved timetable</button><button id="planner-download">Download timetable data</button></div><div id="planner-content"></div>`;
...
async function load(){
 ...
 if(updated){
  ...
  message('Programme update preview: the old mathematical-foundations plan becomes M100, outside the degree. Its dates and adjustments are preserved; any oldest-version separate final assessments become the combined EMA. The old A101 co-study dates are retained as withdrawn history, not a Semester 2 plan. M101 now means Mathematics for physics I and receives none of the old bridge records. Your online records remain unchanged until you accept.');
...
function crossPreparationNotice(){
 const notes=crossModuleWarnings(modules,data.plans,module?.code);
 return notes.length?`<div class="planner-warning"><strong>Companion-module preparation</strong><ul>${notes.map(n=>`<li>${e(n)}</li>`).join('')}</ul><p>Dates are planning evidence, not a mastery check. Changes in either module may require moving dependent work.</p></div>`:'';
}
function render(){
 ...
 const retained=data.retiredPlans?.length?`<p class="notice">${data.retiredPlans.length} withdrawn timetable(s) retained privately for reference. They do not appear as current study dates. Download timetable data to keep or inspect the complete records.</p>`:'';
 $('planner-content').innerHTML=retained+(module?`
 <p>${e(module.durationNote)} <a href="${e(module.durationSource)}">OU reference</a>.</p>
 <p>${module.outsideDegree?'Optional bridge · outside degree credits. ':''}<a href="/programme/#degree-calendar">Degree calendar →</a></p><p><strong>${module.weeks} study weeks · ${module.credits*10} hours · ${(module.credits*10/module.weeks).toFixed(1)} hours/week on average.</strong> Workload varies by week. Your original timetable remains visible after personal changes.</p>
 ...
 ${crossPreparationNotice()}${calendar(rows)}
 ...
 `:`<p>Enrolled modules and any optional bridge plans share this calendar; the bridge does not count towards degree credits. Unit study windows and assessment dates link back to the module planner.</p><label><input id="show-planned" type="checkbox" ${showPlanned?'checked':''}> Include planned modules (not yet enrolled)</label><p>${modules.filter(m=>data.plans[m.code]?.status==='enrolled').length} enrolled modules. ${modules.filter(m=>data.plans[m.code]?.status==='planned').length} saved plans.</p>${crossPreparationNotice()}${rows.length?calendar(rows):'<p>No enrolled module dates to show yet. A saved draft plan appears when you select “Include planned modules”.</p>'}<ul>${modules.map(m=>`<li><a href="${m.path}#study-planner">${e(m.code+' · '+m.title)}${m.outsideDegree?' (outside degree)':''}</a>${data.plans[m.code]?` · ${e(data.plans[m.code].status)} · ${fmt(data.plans[m.code].start)}`:' · no personal dates yet'}</li>`).join('')}</ul><p class="small">The calendar reads the same private dates as each module. Refresh this page to load changes saved in another tab.</p>`);
 bind(edit);
}
