...
export function renderLearning({root,shell,link,e}) {
 ...
 const blockTrees=m100.blocks.map(b=>branch(e(b.id+' · '+b.title),b.units.map(id=>{
  ...
  return branch(e(id+' · '+u.title),`<p>${e(u.can)}</p>${b.id==='B01'?`<p>${link(unitPath(id),'Unit overview')}</p>`:''}${ls.length?ls.map(l=>branch(e(l.title),l.id==='U01-L01'?`<p>${link(lessonPath,'Open lesson')} · Sections 1–2 available; lesson in progress.</p><ul>${openingSections.sections.map((s,i)=>`<li>${published.includes(sectionId(s))?link(lessonPath+'#'+sectionId(s),s.title):e(s.title)+' · '+pending}</li>`).join('')}</ul>`:`<p>${pending} · ${l.hours} planned study hours.</p>`)).join(''):`<p>${pending} · ${link('/programme/bridge/lu-m100/#'+id,'Read the unit blueprint')}</p>`}`);
 }).join(''),b.id));
 ...
 for(const u of m100.units.slice(0,4))pages.push([unitPath(u.id),u.title,[...crumbs,`<span aria-current="page">${u.id}</span>`],intro(u.title,e(u.can))+`<p>${u.hours} planned study hours · ${link('/programme/bridge/lu-m100/#'+u.id,'Unit blueprint')}.</p>`+tree(block1Lessons.lessons.filter(l=>l.unit===u.id).map(l=>branch(e(l.id+' · '+l.title),`<p>${e(l.purpose)}</p><p>${l.id==='U01-L01'?link(lessonPath,'Open lesson · Sections 1–2 available'):pending}</p>`)).join(''))]);
 pages.push([lessonPath,'Signed quantities and ordered calculations',[...crumbs,link(unitPath('U01'),'Unit 1'),'<span aria-current="page">Lesson 1</span>'],intro('Signed quantities and ordered calculations','Start with familiar arithmetic, then build a dependable way of explaining and checking it.')+`<p>Full lesson: 4 hours · Available now: Sections 1–2, about 60 minutes combined.</p><nav aria-label="Lesson contents">${tree(branch('Lesson contents',`<ol>${openingSections.sections.map((s,i)=>`<li>${published.includes(sectionId(s))?link('#'+sectionId(s),s.title):e(s.title)+' · '+pending}</li>`).join('')}</ol>`))}</nav>`+published.map(id=>fs.readFileSync(root+'content/m100-u01-l01-'+id.toLowerCase()+'.html','utf8')).join('')+`<nav class="lesson-pagination" aria-label="Previous and next"><a href="${unitPath('U01')}">← Unit overview</a><span>Sections 3–6: forthcoming</span></nav><p class="small">Original LibraUni teaching · Published 26 September 2026. ${link('/programme/bridge/lu-m100/#opening-sections','Lesson design and section blueprint')}.</p>`]);
